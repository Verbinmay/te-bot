import {
  MS_SEND_ID_AND_UPDATE_QUESTION,
  MS_WRONG_ID_QUESTION,
  MS_WRONG_TEXT_QUESTION,
...
@Scene(CHANGE_QUESTION_ADMIN_SCENE)
export class Q_ChangeQuestionScene {
  constructor(
  ...
  async sceneEnter(@Ctx() ctx: ContextSceneType) {
    try {
      await ctx.reply(MS_SEND_ID_AND_UPDATE_QUESTION, {
        reply_markup: {
  ...
  async textHandle(@Ctx() ctx: ContextSceneType) {
    try {
      ...
      if (text == BACK_TO_MAIN_MENU) {
        ...
        const [questionId, newQuestion] = text.split('|');
        ...
        if (newQuestion.trim().length < 3) {
          await ctx.reply(MS_WRONG_TEXT_QUESTION);
          await ctx.scene.enter(CHANGE_QUESTION_ADMIN_SCENE);
          return;
        }
        ...
        question.body = newQuestion.trim();
